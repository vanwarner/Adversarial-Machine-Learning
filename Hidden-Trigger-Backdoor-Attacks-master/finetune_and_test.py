...
all_classes_wnid_list = options["all_classes_wnid_list"].format(experimentID)
...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, is_inception=False):
	...
	shrinkpad_val_acc_arr = np.zeros(num_epochs)
	shrinkpad_acc_arr = np.zeros(num_epochs)
	flip_val_acc_arr = np.zeros(num_epochs)
	flip_acc_arr = np.zeros(num_epochs)
	...
	for epoch in range(num_epochs):
		...
		for phase in ['train', 'test', 'notpatched', 'patched', 'shrinkpad_val', 'shrinkpad', 'flip_val', 'flip']:
			if phase == 'train':
				...
				for inputs, labels in tqdm(dataloaders[phase]):
					...
					if phase == 'flip':
						for z in range(inputs.size(0)):
							...
							inputs[z, :, start_y:start_y+patch_size,
			...
			if phase == 'shrinkpad_val':
				shrinkpad_val_acc_arr[epoch] = epoch_acc
			if phase == 'shrinkpad':
				shrinkpad_acc_arr[epoch] = epoch_acc
			if phase == 'flip_val':
				flip_val_acc_arr[epoch] = epoch_acc
			if phase == 'flip':
				flip_acc_arr[epoch] = epoch_acc
			# deep copy the model
...
'''
	...
	logging.info('10 Epochs with Best Acc- ShrinkPad Validation Accuracy: Mean {:.3f} Std {:.3f} '
				 .format(shrinkpad_val_acc_arr[top10_idx].mean(),shrinkpad_val_acc_arr[top10_idx].std()))	
	logging.info('10 Epochs with Best Acc- ShrinkPad Targeted Attack Success Rate: Mean {:.3f} Std {:.3f} '
				 .format(shrinkpad_acc_arr[top10_idx].mean(),shrinkpad_acc_arr[top10_idx].std()))	
	logging.info('10 Epochs with Best Acc- Flip Validation Accuracy: Mean {:.3f} Std {:.3f} '
				 .format(flip_val_acc_arr[top10_idx].mean(),flip_val_acc_arr[top10_idx].std()))	
	logging.info('10 Epochs with Best Acc- Flip Validation Targeted Attack Success Rate: Mean {:.3f} Std {:.3f} '
				 .format(flip_acc_arr[top10_idx].mean(),flip_acc_arr[top10_idx].std()))	

	# save meta into pickle
	meta_dict = {'Val_acc': test_acc_arr,
				 'Patched_acc': patched_acc_arr,
				 'NotPatched_acc': notpatched_acc_arr,
				 'ShrinkPad_val_acc': shrinkpad_val_acc_arr,
				 'ShrinkPad_acc': shrinkpad_acc_arr,
				 'Flip_val_acc': flip_val_acc_arr,
				 'Flip_acc': flip_acc_arr,
				 }
...
def shrink_and_pad(im, padding_amount):
...
class ShrinkPad:
	def __init__(self, padding_amount=30):
		self.padding_amount = padding_amount

	def __call__(self, img):
		return shrink_and_pad(img, self.padding_amount)


if __name__ == '__main__':
	for _ in range(1): #10-fold validation
		...
		shrinkpad_transforms = transforms.Compose([
				transforms.Resize((input_size, input_size)),
				ShrinkPad(),
				transforms.ToTensor(),
		])
		...
		defense_transforms = transforms.Compose([
				transforms.Resize((input_size, input_size)),
				transforms.RandomHorizontalFlip(1),
				ShrinkPad(),
				transforms.ToTensor(),
		...
		with open("data/{}/finetune_filelist.txt".format(experimentID), "w") as f1:
			with open(all_classes_wnid_list) as f2:
		...
		with open("data/{}/patched_filelist.txt".format(experimentID), "w") as f1:
			...
			#Metric is ASR, this dataset tests the source wnid against the label of the target class
			else:
				for i, source_wnid in enumerate(source_wnids):
					with open("ImageNet_data_list/test/" + source_wnid + ".txt", "r") as f2:
						lines = f2.readlines()
		...
		dataset_shrinkpad_test = LabeledDataset(clean_data_root + "/val",
										"data/{}/test_filelist.txt".format(experimentID), shrinkpad_transforms)
		dataset_shrinkpad = LabeledDataset(clean_data_root + "/val",
										"data/{}/patched_filelist.txt".format(experimentID), shrinkpad_transforms)
		dataset_flip_test = LabeledDataset(clean_data_root + "/val",
										"data/{}/test_filelist.txt".format(experimentID), flip_transforms)
		dataset_flip = LabeledDataset(clean_data_root + "/val",
										"data/{}/patched_filelist.txt".format(experimentID), flip_transforms)
		...
		dataloaders_dict['notpatched'] =  torch.utils.data.DataLoader(dataset_patched, batch_size=batch_size,
																	shuffle=False, num_workers=4)
		dataloaders_dict['shrinkpad_val'] =  torch.utils.data.DataLoader(dataset_shrinkpad_test, batch_size=batch_size,
																	shuffle=False, num_workers=4)
		dataloaders_dict['shrinkpad'] =  torch.utils.data.DataLoader(dataset_shrinkpad, batch_size=batch_size,
																	shuffle=False, num_workers=4)
		dataloaders_dict['flip_val'] =  torch.utils.data.DataLoader(dataset_flip_test, batch_size=batch_size,
																	shuffle=False, num_workers=4)
		dataloaders_dict['flip'] =  torch.utils.data.DataLoader(dataset_flip, batch_size=batch_size,
																	shuffle=False, num_workers=4)																
